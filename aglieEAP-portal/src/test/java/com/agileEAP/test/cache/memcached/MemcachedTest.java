package com.agileEAP.test.cache.memcached;
...
import com.agileEAP.module.cache.memcached.SpyMemcachedClient;
...
public class MemcachedTest extends AbstractJUnit4SpringContextTests {
	...
	public void normalObject() {
		...
		String key = "tradeRecord:1";
		TradeRecord tradeRecord = new TradeRecord();
		tradeRecord.setTranCode("trade"+UUID.randomUUID());
		tradeRecord.setGoodsCode("goodsCode"+UUID.randomUUID());
		
		spyMemcachedClient.set(key, 60 * 60 * 1, tradeRecord);

		TradeRecord result = spyMemcachedClient.get(key);
		assertEquals(tradeRecord, result);
